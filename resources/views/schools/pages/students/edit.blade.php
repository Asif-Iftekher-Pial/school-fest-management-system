	...
	<div class="row">
		...
		<div class="col-9">
			<div class="m-3">
				...
				<form action="{{ url('/students/update/'.$data->id) }}" method="Post" enctype="multipart/form-data" accept-charset="UTF-8">
					{{ method_field('PATCH') }}
					{{ csrf_field() }}
					<div class="row">
						<div class="col-6">
						...
						<div class="col-6">
							<div class="form-group">
							    ...
							    <select class="form-control" name="studentgroup_id">
							        ...
							        @foreach($sgroup as $items)
							            <option value="{{$items->id }}" {{ $items->id == $data->studentgroup_id ? 'selected' : '' }}>{{ $items->title }}</option>
							        @endforeach
							    </select>
							</div>
							<div class="form-group">
							...
							<div class="form-group">
							    ...
							    <select class="form-control demo3 demo4" name="group_id">
							        <option value="{{ $data->group->id }}">{{ $data->group->title }}</option>
							    </select>
							</div>
							<div class="form-group">
